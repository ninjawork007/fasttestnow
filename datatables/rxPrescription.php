...
// EOT;
$table = <<<EOT
SELECT id, firstname, lastname, shipping_service, phone, email, semaglutide_check, quantity, insulin_qty, notes, handled_at, pdfLink FROM  tbl_rx_order
EOT;
/** Table's primary key */
$primaryKey = 'id';
...
$columns = array(
    ...
    array('db' => 'shipping_service', 'dt' => 2),
    array('db' => 'phone', 'dt' => 3),
    array('db' => 'email', 'dt' => 4),
    array('db' => 'semaglutide_check', 'dt' => 5),
    array('db' => 'quantity', 'dt' => 6),
    array('db' => 'insulin_qty', 'dt' => 7),
    array('db' => 'notes', 'dt' => 8),
    array('db' => 'handled_at', 'dt' => 9),
    array('db' => 'pdfLink', 'dt' => 10),
    array('db' => 'lastname', 'dt' => 11),
);
...
$where = "";
$orderBy = ($_REQUEST['order'][0]['column'] == 0) ? "ORDER BY tbl_rx_order.id DESC" : null;
