...
class Rsv {
    ...
    function insert($keys) {
        ...
        if($this->isTable() === true) {
            ...
            $params = implode("`, `", $keys);
            $sql = "INSERT INTO tbl_rsv (`".$params."`) VALUES ('".$importData."')";
            
            $q = mysqli_query($conn, $sql);
            $id = mysqli_insert_id($conn);
            if ($q === true) {
                $sql = "UPDATE tbl_rsv SET `user_token`='".$this->userToken($id)."' WHERE id=". $id;
            ...
            var_dump("Failed in Creating tbl_rsv Table"); die;
        }
    }

    function userToken($id) {
        $conn = dbCon(); 
        $maxTokenQ = "SELECT user_token FROM `tbl_rsv` WHERE id NOT IN ($id) ORDER BY id DESC LIMIT 0,1";
    ...
    function update($id = NULL) {
        ...
        $sql = "UPDATE tbl_rsv SET ".$this->data." WHERE id=". $id;
    ...
    function delete($id = NULL) {
        ...
        $sql = "DELETE FROM tbl_rsv WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function findById($id = NULL) {
        ...
        $sql = "SELECT * FROM  tbl_rsv WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function generatePDF($id) {
        ...
        $data = $this->data;
        $output = "../uploads/c742a402367f2030636af25ce0c9713ersv" . $id . ".pdf";
        ...
        $awsPdfFile = "8513da060dea559ed3dff467ecb2651ersv" . $id . ".pdf";

        $url = $this->uploadReportToAwsS3($output, $awsPdfFile);
        
        $conn = dbCon();
        $sql = "UPDATE tbl_rsv SET pdfLink='".$url."' WHERE id=". $id;
        $q = mysqli_query($conn, $sql);
    ...
    function sendEmail($data, $URL) {
        ...
        $subject = "Rsv Screening [" . $data['firstname'] . " " . $data['lastname'] . "]";
