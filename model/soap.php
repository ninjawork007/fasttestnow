...
class Soap {
    ...
    function create() {
        $conn = dbCon();
		$sql = "CREATE TABLE tbl_soap (
            id INT(11) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
            appointment_id INT(11) NOT NULL,
            user_token VARCHAR(30) NOT NULL DEFAULT 0,
            qualified VARCHAR(30) NOT NULL,
            soap_check VARCHAR(30) NOT NULL,
            allergies VARCHAR(50),
            subjective INT(2) NOT NULL,
            objective INT(2) NOT NULL,
    ...
    function insert($keys) {
        ...
        if($this->isTable() === true) {
            ...
            $params = implode("`, `", $keys);
            $sql = "INSERT INTO tbl_soap (`".$params."`) VALUES ('".$importData."')";
            
            $q = mysqli_query($conn, $sql);
            $id = mysqli_insert_id($conn);
            if ($q === true) {
                $sql = "UPDATE tbl_soap SET `user_token`='".$this->userToken($id)."' WHERE id=". $id;
            ...
            var_dump("Failed in Creating tbl_soap Table"); die;
        }
    }

    function userToken($id) {
        $conn = dbCon(); 
        $maxTokenQ = "SELECT user_token FROM `tbl_soap` WHERE id NOT IN ($id) ORDER BY id DESC LIMIT 0,1";
    ...
    function update($id = NULL) {
        ...
        $sql = "UPDATE tbl_soap SET ".$this->data." WHERE id=". $id;
    ...
    function delete($id = NULL) {
        ...
        $sql = "DELETE FROM tbl_soap WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function findById($id = NULL) {
        ...
        $sql = "SELECT * FROM  tbl_soap WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function generatePDF($id) {
        ...
        $data = $this->data;
        $output = "../uploads/c742a402367f2030636af25ce0c9713esoap" . $id . ".pdf";
        ...
        $awsPdfFile = "8513da060dea559ed3dff467ecb2651esoap" . $id . ".pdf";

        $url = $this->uploadReportToAwsS3($output, $awsPdfFile);
        
        $conn = dbCon();
        $sql = "UPDATE tbl_soap SET pdfLink='".$url."' WHERE id=". $id;
        $q = mysqli_query($conn, $sql);
    ...
    function sendEmail($data, $URL) {
        ...
        $receiver = $data['email'];
        $pdf_name = "Soap Screening";
        $subject = "Soap Screening [" . $data['firstname'] . " " . $data['lastname'] . "]";
