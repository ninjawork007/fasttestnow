...
class Thyroid {
    ...
    function insert($keys) {
        ...
        if($this->isTable() === true) {
            ...
            $params = implode("`, `", $keys);
            $sql = "INSERT INTO tbl_thyroid (`".$params."`) VALUES ('".$importData."')";
            
            $q = mysqli_query($conn, $sql);
            $id = mysqli_insert_id($conn);
            if ($q === true) {
                $sql = "UPDATE tbl_thyroid SET `user_token`='".$this->userToken($id)."' WHERE id=". $id;
            ...
            var_dump("Failed in Creating tbl_thyroid Table"); die;
        }
    }

    function userToken($id) {
        $conn = dbCon(); 
        $maxTokenQ = "SELECT user_token FROM `tbl_thyroid` WHERE id NOT IN ($id) ORDER BY id DESC LIMIT 0,1";
    ...
    function update($id = NULL) {
        ...
        $sql = "UPDATE tbl_thyroid SET ".$this->data." WHERE id=". $id;
    ...
    function delete($id = NULL) {
        ...
        $sql = "DELETE FROM tbl_thyroid WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function findById($id = NULL) {
        ...
        $sql = "SELECT * FROM  tbl_thyroid WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function generatePDF($id) {
        ...
        $data = $this->data;
        $output = "../uploads/c742a402367f2030636af25ce0c9713ethyroid" . $id . ".pdf";
        ...
        $awsPdfFile = "8513da060dea559ed3dff467ecb2651ethyroid" . $id . ".pdf";

        $url = $this->uploadReportToAwsS3($output, $awsPdfFile);
        
        $conn = dbCon();
        $sql = "UPDATE tbl_thyroid SET pdfLink='".$url."' WHERE id=". $id;
        $q = mysqli_query($conn, $sql);
    ...
    function sendEmail($data, $URL) {
        ...
        $subject = "Thyroid Screening [" . $data['firstname'] . " " . $data['lastname'] . "]";
