...
class Hiv {
    ...
    function insert($keys) {
        ...
        if($this->isTable() === true) {
            ...
            $params = implode("`, `", $keys);
            $sql = "INSERT INTO tbl_hiv (`".$params."`) VALUES ('".$importData."')";
            
            $q = mysqli_query($conn, $sql);
            $id = mysqli_insert_id($conn);
            if ($q === true) {
                $sql = "UPDATE tbl_hiv SET `user_token`='".$this->userToken($id)."' WHERE id=". $id;
            ...
            var_dump("Failed in Creating tbl_hiv Table"); die;
        }
    }

    function userToken($id) {
        $conn = dbCon(); 
        $maxTokenQ = "SELECT user_token FROM `tbl_hiv` WHERE id NOT IN ($id) ORDER BY id DESC LIMIT 0,1";
    ...
    function update($id = NULL) {
        ...
        $sql = "UPDATE tbl_hiv SET ".$this->data." WHERE id=". $id;
    ...
    function delete($id = NULL) {
        ...
        $sql = "DELETE FROM tbl_hiv WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function findById($id = NULL) {
        ...
        $sql = "SELECT * FROM  tbl_hiv WHERE id=".$id;
        $q = mysqli_query($conn, $sql);
    ...
    function generatePDF($id) {
        ...
        $data = $this->data;
        $output = "../uploads/c742a402367f2030636af25ce0c9713ehiv" . $id . ".pdf";
        ...
        $awsPdfFile = "8513da060dea559ed3dff467ecb2651ehiv" . $id . ".pdf";

        $url = $this->uploadReportToAwsS3($output, $awsPdfFile);
        
        $conn = dbCon();
        $sql = "UPDATE tbl_hiv SET pdfLink='".$url."' WHERE id=". $id;
        $q = mysqli_query($conn, $sql);
    ...
    function sendEmail($data, $URL) {
        ...
        $subject = "Hiv Screening [" . $data['firstname'] . " " . $data['lastname'] . "]";
